...
const NotificationCard = ({data, index, notificationState}) => {
  ...
  let {seen, type, reply, createdAt, comment, replied_on_comment, user, user:{personal_info:{fullname, username, profile_img}}, blog:{blog_id, title}, _id:notification_id} =data;

  let { userAuth } = useContext(UserContext) || {};
  let {  username:author_username, profile_img:author_profile_img, access_token } = userAuth || {};
...
<span className="font-normal">
  {
    ...
    "replied on"
          ...
          {
             type == 'reply' ?
             <div className="mt-4 p-4 rounded-md bg-grey">
              <p>{replied_on_comment.comment}</p>
             </div>
             : <Link to={`/blog/${blog_id}`} className="text-dark-grey font-medium hover:underline line-clamp-1">{`"${title}"`}</Link>
          }
        </div>
      </div>
      {
...
<p>{getDay(createdAt)}</p>
{
  ...
  <>
  {
    !reply ? <button className="underline hover:text-black" onClick={handleReplyClick}>
    Reply
      </button> : ""
  }
  <button className="underline hover:text-dark-grey" onClick={(e)=>handleDelete(comment._id, "comment", e.target )}>Delete</button>
  </> : ""
}
      </div>
      {
...
<button className="underline hover:text-dark-grey ml-14 mt-2" onClick={(e)=>handleDelete(comment._id, "reply", e.target )}>Delete</button>
