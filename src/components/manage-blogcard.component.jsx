...
<div className="flex flex-col justify-between py-2 w-full min-w-[300px]">
  <div>
    ...
    <Link to={`/blog/${blog_id}`}className="blgo-title mb-4 hover:underline">{title}</Link>
    <p className="line-clamp-1">Published on {getDay(publishedAt)}</p>
  </div>
  <div className="flex gap-6 mt-3">
<Link to={`/editor/${blog_id}`} className="pr-4 py-2 underline">Edit</Link>
<button className="block lg:hidden" onClick={()=> setShowStat(preVal => !preVal)}>Stats</button>
...
const deleteBlog = (blog, access_token, target) => {
  ...
  axios
  .post(import.meta.env.VITE_SERVER_DOMAIN + "/delete-blog", {
    blog_id}, {
      headers: {
