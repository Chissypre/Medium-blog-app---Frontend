...
const CommentCard = ({index,leftVal, commentData }) => {
  ...
  let {blog, blog: {comments, activity, activity:{total_parent_comments}, comments: {results: commentsArr}, author:{personal_info:{username:blog_author}}}, 
...
setBlog({...blog, comments:{results:commentsArr}, activity:{...activity, total_parent_comments:total_parent_comments -(
  commentData.childrenLevel === 0 && isDelete
)}})
 }
...
if (commentsArr[currentIndex].children.length){
 ...
 const deleteComment = (e) =>{
  ...
  axios
  .post(import.meta.env.VITE_SERVER_DOMAIN + "/delete-comment", {
    _id
  }, {
    headers: {
...
className="text-dark-grey p-2 px-3 hover:bg-grey/30 rounded-md flex items-center gap-2">
  ...
  return (
    ...
    <Toaster/>
    <div className="py-1">
    <div className="w-full h-auto mb-1 " style={{ paddingLeft: `${leftVal * 10}px` }}>
    <div className="my-5 p-6 rounded-md border-grey">
      ...
      <div className="flex gap-5 items-center mt-5">
        {
          commentData.isReplyLoaded ? <button className="text-dark-grey p-2 px-3 hover:bg-grey/30 rounded-md flex
          items-center gap-2" onClick={hideReplies}>Hide Reply</button> : 
          
         <button className="text-dark-grey p-2 px-3 hover:bg-grey/30 rounded-md flex
         items-center gap-2" onClick={loadReplies}>
{children.length} Reply
        ...
        <button className="underline" onClick={handleReplyClick}>Reply</button>
        {
          ...
          <button className="p-2 px-3 rounded-md border border-grey ml-auto hover:bg-red/30 hover:text-red 
          flex items-center" onClick={deleteComment}>
